...
#for sn in write_only read_write read_write_k update_index update_non_index
...
do
	...
	do
		...
		echo "|| KunlunBase || $sumthd || $sumtps || $sumqps || $avgavg || $maxp95 ||" >> allresult.txt
	done
	echo '----' >> allresult.txt
done
