...
class meta:
    ...
    def show_shard_nodes(self, shard_id):
        sql = "select hostaddr, port, user_name, passwd, member_state from shard_nodes where status = 'active' and shard_id = %s;" % shard_id
        res = self.get_info(sql)
        return res


class pgsql:
    def __init__(self, node_info_list, db, autocommit=0):
        node_info = node_info_list
        conn = psycopg2.connect(host=node_info[0], port=int(node_info[1]), user=node_info[2], password=node_info[3], dbname=db)
        if autocommit == 1:
    ...
    def sql_with_res(self, sql, commit=1):
        conn = self.conn
        cur = self.cur
        cur.execute(sql)
        res = cur.fetchall()
