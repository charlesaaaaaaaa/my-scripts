...
def rcr_cdc(db, engine='InnoDB', partition=0, times=1):
    ...
    try:
        ...
        if db == 'mysql' or db == 'mariadb':
            ...
            res = bin.load.mongo.LoadMg().multi_load(thread=10, each_thread_size=200000, tb=tb)
            assert res != 0
        elif db == "es":
            ...
            # 先去备es上创建一样的index
            bin.load.es.LoadEs().delete_create_index(host_num=1)
            # 进程数10个，每个各灌1000000，为什么是thread_size, 因为我懒得改参数名了
            bin.load.es.LoadEs().mutil_load(drop=1, each_thread_size=1000000, process=10)
...
if __name__ == "__main__":
    ...
    ps.add_argument('--partition', help='1为要进行分区，0为不要进行分区，[0]|[1], db为多'
                                        '个时这个也要用半角逗号","分隔且一一对应上', default='0, 1')
    args = ps.parse_args()
    print(args)
