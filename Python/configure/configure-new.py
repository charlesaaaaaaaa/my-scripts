...
def configs():
    ...
    for i in CompIp:
        for a in range(0, CompNum):
            if Compvalues[a] :
                ...
                if types == 'one_click':
                    ...
                    BashStmt = "ssh %s@%s \'echo %s = %s >> %s/server_datadir/instance_data/comp_datadir/%s/postgresql.conf\'" % (defuser, SCompIp, SCompkeys, SCompvalues, defbase, SCompIp)
                
                elif types == 'cluster_mgr':
                    BashStmt = "ssh %s@%s \'echo %s = %s >> %s/comp_datadir/%s/postgresql.conf\'" % (defuser, SCompIp, SCompkeys, SCompvalues, serDat, SCompIp)
        ...
        if types == 'one_click':
            ...
            stmt = "ssh %s@%s \'%s/instance_binaries/computer/%s/kunlun-server-0.9.2/bin/pg_ctl reload -D %s/server_datadir/instance_data/comp_datadir/%s/postgresql.conf\'" % (defuser, SCompIp, defbase, SCompIp, defbase, SCompIp) 

        elif types == 'cluster_mgr':
            stmt = "ssh %s@%s \'%s/computer/%s/kunlun-server-0.9.2/bin/pg_ctl reload -D %s/server_datadir/instance_data/comp_datadir/%s/postgresql.conf\'" % (defuser, SCompIp, insBin, SCompIp, insBin, SCompIp)
