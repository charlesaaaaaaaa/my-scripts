...
for i in CompIp:
    for a in range(0, CompNum):
        if Compvalues[a] :
            ...
            of=open('config.sh','a')
            AddLine = "line=`cat %s/postgresql.conf aws -F= '\\'{print \\$1}\\'' | grep -n -w '\\'^%s\\'' | awk -F: '\\'{print \\$1}\\''` && " % (SCompDir,SCompkeys)
...
for i in MetaIp:
    for a in range(0, MedaNum):
        if Medavalues[a]:
            ...
            of=open('config.sh','a')
            AddLine = "line=`cat %s/%s/my_%s.cnf | aws -F= '\\'{print \\$1}\\'' | grep -n -w '\\'^%s\\'' | awk -F: '\\'{print \\$1}\\''` && " % (SMetaDir, SMetaPort, SMetaPort, SMedakeys)
...
for i in DataIp:
    for a in range(0, MedaNum):
        if Medavalues[a]:
            ...
            of=open('config.sh','a')
            AddLine = "line=`cat  %s/%s/my_%s.cnf | aws -F= '\\'{print \\$1}\\'' | grep -n -w '\\'^%s\\'' | awk -F: '\\'{print \\$1}\\''` && " % (SDataDir, SDataPort, SDataPort, SMedakeys)
