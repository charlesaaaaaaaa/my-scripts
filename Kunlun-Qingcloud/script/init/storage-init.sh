...
master_info=`cat /home/ubuntu/a.txt | grep /self/hosts/data_node/ | grep "$self_gid"$ | grep gid | awk '{print $1}' | sed 's/...$//'` #获取本节点对应的主节点的前缀info
...
if [ $self_ip != $master_ip ] ; then
	...
	sudo python2 /home/ubuntu/kunlun/kunlun-storage/dba_tools/install-mysql.py --config=./mgr_config.json --target_node_index=1
	...
	sudo sed -i "0,/127.0.0.1/s/127.0.0.1/${master_ip}/" /home/ubuntu/kunlun/kunlun-storage/dba_tools/mgr_config.json
        ...
        sudo python2 /home/ubuntu/kunlun/kunlun-storage/dba_tools/install-mysql.py --config=./mgr_config.json --target_node_index=0
