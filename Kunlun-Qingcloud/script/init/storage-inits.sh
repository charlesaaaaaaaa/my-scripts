...
master_info=`cat /home/ubuntu/tmp/a.txt | grep /self/hosts/data_node/ | grep "$self_gid"$ | grep gid | awk '{print $1}' | sed 's/...$//'` #获取本节点对应的主节点的前缀info
...
cat /home/ubuntu/tmp/a.txt | grep gid | grep "$self_gid"$ | grep self/hosts/data_node-re |awk '{print $1}' | sed 's/...$//' > /home/ubuntu/tmp/replica.txt
...
innolwt=`cat /home/ubuntu/tmp/a.txt | grep data_node/innodb_lock_wait_timeout | awk '{print $2}' | sed -n '1p'`
lockt=`cat /home/ubuntu/tmp/a.txt | grep data_node/lock_wait_timeout | awk '{print $2}' | sed -n '1p'`
