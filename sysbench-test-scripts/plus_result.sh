...
touch allresult.txt
echo $#
for sn in point_select insert read_only read_write write_only update_index update_non_index
do
	...
	do
		for i in $*
		do
			export thd$i=`cat $i/result | grep -A 4 "$sn" | grep -v "$sn" | grep -v threads | sed -n "${tn}p" | awk '{print $4}'`
			export tps$i=`cat $i/result | grep -A 4 "$sn" | grep -v "$sn" | grep -v threads | sed -n "${tn}p" | awk '{print $6}'`
			export qps$i=`cat $i/result | grep -A 4 "$sn" | grep -v "$sn" | grep -v threads | sed -n "${tn}p" | awk '{print $8}'`
			export avg$i=`cat $i/result | grep -A 4 "$sn" | grep -v "$sn" | grep -v threads | sed -n "${tn}p" | awk '{print $10}'`
			export p95$i=`cat $i/result | grep -A 4 "$sn" | grep -v "$sn" | grep -v threads | sed -n "${tn}p" | awk '{print $12}'`
		done
