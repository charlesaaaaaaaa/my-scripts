...
sed -i 's/$/ ||/' resault
#sed -i '1 i\|| db || threads || read || wirte || txn || total || avg response time(ms) || .95 response time(ms) || total events ||' resault
...
do
        ei=`expr ${i} \* 13 + 4`
	ai=`expr ${i} \* 13 + 5 `
	sed -i "$ai i\|| db || threads || read || wirte || txn || avg response time(ms) || .95 response time(ms) || total events ||" resault
