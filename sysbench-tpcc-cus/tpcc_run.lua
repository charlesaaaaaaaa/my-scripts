...
-- TPCC-like workload
-- ----------------------------------------------------------------------
--package.path ="/home/charles/MyTools/Packages/sysbench-tpcc-cus/tpcc_common-fs.lua"..";..\\?.lua"
require("tpcc_common")
...
--
--local next = tostring(os.time()):reverse():sub(4,12)
...
print('------district------\nd_next_o_id: ',d_next_o_id,'\n  table_num: ',table_num,'\n       d_id: ',d_id,'\n       w_id: ',w_id,'\n------district------\n')
  con:query(([[UPDATE district%d  
                  SET d_next_o_id = %d
                WHERE d_id = %d AND d_w_id= %d]]):format(table_num, d_next_o_id + 1, d_id, w_id))  --con:query(([[UPDATE district%d

rand10000=math.random(1,9000000)
r1w=d_next_o_id+rand10000
...
--                       :o_ol_cnt, :o_all_local);
  ...
  con:query(([[INSERT INTO orders%d
                    ...
                    format(table_num, r1w, d_id, w_id, c_id, ol_cnt, all_local))
...
--    VALUES (:o_id,:d_id,:w_id); */
  ...
  con:query(([[INSERT INTO new_orders%d (no_o_id, no_d_id, no_w_id)
                    VALUES (%d,%d,%d)]]):
                   format(table_num, r1w, d_id, w_id))
...
--		ORDER BY c_first;
	...
	rs = con:query(([[SELECT c_id
			   ...
			   WHERE c_w_id = %d AND c_d_id= %d
                             ORDER BY c_first]]
			):format(table_num, c_w_id, c_d_id ))
    r10=math.random(10)
--	for i = 1,  (namecnt / 2 ) + r10 do
--		row = rs:fetch_row()
--		c_id = row[1]
--	end
  end -- byname
  ...
  c_phone, c_credit, c_credit_lim, c_discount, c_balance, c_ytd_payment, c_since =
	 con:query_row(([[SELECT c_first, c_middle, c_last, c_street_1,
			   ...
			   WHERE c_w_id = %d 
			     AND c_d_id= %d
			     AND c_id=%d ]])
			 :format(table_num, c_w_id, c_d_id, c_id ))  --FROM customer%d

    print('\nthis is table_num: ',table_num)
    print('this is c_w_id: ',c_w_id)
    print('this is c_d_id: ',c_d_id)
    print('this is --c_id: ',c_id)
    print('this is c_balance: ',c_balance)
    print('this is tonumber(c_balance): ',tonumber(c_balance),'\n')
