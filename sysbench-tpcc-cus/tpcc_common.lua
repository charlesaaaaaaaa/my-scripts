...
-- HISTORY TABLE
   ...
   query = string.format([[
	...
	h_data varchar(24) %s,
    PRIMARY KEY(h_c_id, h_d_id ,h_c_w_id)
	) %s %s]],
...
-- STOCK table
   ...
   for j = 1 , MAXITEMS do
      local i_im_id = sysbench.rand.uniform(1,100)
      local i_price = sysbench.rand.uniform_double()*10+1
...
-- STOCK table
   ...
   for ol_id = 1, a_counts[warehouse_num][d_id][o_id] do 
      ...
      query = string.format([[(%d, %d, %d, %d, %d, %d, %s, 5, %f, '%s' )]],
        ...
        o_id < 2101 and 0 or sysbench.rand.uniform_double()*9999,--o_id < 2101 and 0 or sysbench.rand.uniform_double()*9999.99
	string.rep(sysbench.rand.string("@"),24)
        )
