...
then
	...
	do
		echo $i times
		./tpcc.lua --db-driver=pgsql --pgsql-host=$host --mysql-ignore-errors=1062 --pgsql-port=$port --pgsql-user=$user --pgsql-password=$pwds --pgsql-db=$db --time=$time --threads=$thread --report-interval=1 --tables=1 --scale=$wh --trx_level=RC --db-ps-mode=auto  run
		echo 
		echo ==========
		echo $i times
		echo ==========
